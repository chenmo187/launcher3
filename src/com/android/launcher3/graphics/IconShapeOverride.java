...
//            Log.d(TAG, "isSupported: 开发者模式关闭，不支持换图标");
//            return false;
//        }
    ...
    //把默认突变定义为圆角图片 属性值在value/config.xml定义  2021 07 30
    private static String defIcon = "M50,0 C10,0 0,10 0,50 0,90 10,100 50,100 90,100 100,90 100,50 100,10 90,0 50,0 Z";

    private static String getAppliedValue(Context context) {
        //return getDevicePrefs(context).getString(KEY_PREFERENCE, "");
        String string = getDevicePrefs(context).getString(KEY_PREFERENCE, defIcon);
        Log.d(TAG, "getAppliedValue: value " + string);
        return string;
    }
    ...
    private static class OverrideApplyHandler implements Runnable {
        ...
        public void run() {
            ...
            Log.d(TAG, " clean iconCache  kill progress.........");
            // Wait for it
            try {
