...
public class CellLayout extends ViewGroup {
    ...
    private final float mChildScale = 1.0f;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int widthSize = MeasureSpec.getSize(widthMeasureSpec);//794px
        int heightSize = MeasureSpec.getSize(heightMeasureSpec);//451px
        int childWidthSize = widthSize - (getPaddingLeft() + getPaddingRight());//726px
        int childHeightSize = heightSize - (getPaddingTop() + getPaddingBottom());//434px

        if (mFixedCellWidth < 0 || mFixedCellHeight < 0) {
            int cw = DeviceProfile.calculateCellWidth(childWidthSize, mCountX);//145
            int ch = DeviceProfile.calculateCellHeight(childHeightSize, mCountY);//217
//            Log.d(TAG, "onMeasure: 行数:" + mCountY + "  列数:" + mCountX +
//                    "  限制控件宽:" + cw + " 高:" + ch +
//                    "  childWidthSize:" + childWidthSize + "  childHeightSize:" + childHeightSize);
            if (cw != mCellWidth || ch != mCellHeight) {
                mCellWidth = cw;
                mCellHeight = ch;
                // Log.d(TAG, "onMeasure: ----------000----------------widthSize:" + widthSize + "  heightSize:" + heightSize);
                mShortcutsAndWidgets.setCellDimensions(mCellWidth, mCellHeight, mCountX, mCountY);
            }
        }
...
//        Log.d(TAG, "onMeasure: 测量行数:" + mCountY + "  当前列数:" + mCountX +
//                " 测量控件宽:" + newWidth + "  高:" + newHeight +
//                "  Shortcuts maxW:" + maxWidth + "  Shortcuts maxH:" + maxHeight);
        if (mFixedWidth > 0 && mFixedHeight > 0) {
            setMeasuredDimension(maxWidth, maxHeight);
            // Log.d(TAG, "onMeasure: ----------111----------------");
        } else {
            setMeasuredDimension(widthSize, heightSize);
            // Log.d(TAG, "onMeasure: ----------222----------------");
        }
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        left += (int) Math.ceil(getUnusedHorizontalSpace() / 2f);//2f   修改onLayout()中参数未验证
        int right = r - l - getPaddingRight();//r - l - getPaddingRight()
        right -= (int) Math.ceil(getUnusedHorizontalSpace() / 2f);//2f

        int top = getPaddingTop();//getPaddingTop();
        int bottom = b - t - getPaddingBottom();//b - t - getPaddingBottom();
