...
public class AddWorkspaceItemsTask extends BaseModelUpdateTask {
    ...
    public AddWorkspaceItemsTask(List<Pair<ItemInfo, Object>> itemList) {
        mItemList = itemList;
       // Log.d("AddWorkspaceItemsTask", "mItemList size: " + mItemList.size());
    }
    ...
    public void execute(LauncherAppState app, BgDataModel dataModel, AllAppsList apps) {
        ...
        Log.d("AddWorkspaceItemsTask", ">>>>>>开机实际加载总屏数: " + workspaceScreens.size());
        synchronized (dataModel) {
            ...
            for (ItemInfo item : filteredItems) {
                ...
                if (itemInfo != null) {
                  //  Log.d("AddWorkspaceItemsTask", "-------filteredItems dbSave title:" + item.title);
                }
       ...
       // Log.d("AddWorkspaceItemsTask", "处理完毕屏幕数量: " + workspaceScreens.size());
                    ...
                    if (!addedItemsFinal.isEmpty()) {
                        ...
                        for (ItemInfo i : addedItemsFinal) {
                            if (i.screenId == lastScreenId) {
                                addAnimated.add(i);
                               // Log.d("AddWorkspaceItemsTask", " addAnimated title:" + i.title);
                            } else {
                                addNotAnimated.add(i);
                               // Log.d("AddWorkspaceItemsTask", " addNotAnimated title:" + i.title);
                            }
                        }
                    }
    ...
    protected boolean shortcutExists(BgDataModel dataModel, Intent intent, UserHandle user) {
        ...
        synchronized (dataModel) {
            //Log.d("AddWorkspaceItemsTask", "shortcutExists  dataModel size: " + dataModel.itemsIdMap.size());
    ...
    protected Pair<Long, int[]> findSpaceForItem(
        ...
        synchronized (dataModel) {
            // Log.d("AddWorkspaceItemsTask", "----------------------findSpaceForItem: 数据库加载总数:" + dataModel.itemsIdMap.size());
        ...
        int preferredScreenIndex = 0;//add by xiaoyu 默认首页就是0 2021 06 16
        ...
        if (!found) {
            ...
            for (int screen = 0; screen < screenCount; screen++) {//2021 07 12去掉所有的favorite 包括侧边hotset 修改为默认从第一屏index=0开始添加
