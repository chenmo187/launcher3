...
public class ShortcutAndWidgetContainer extends ViewGroup {
    ...
    public void measureChild(View child) {
        ...
        if (child instanceof LauncherAppWidgetHostView) {
            ...
            int cellPaddingY = (int) Math.max(0, ((lp.height - cHeight) / 2f));//原设置
            //int cellPaddingY = (int) Math.max(0, cHeight / 2f);
            // int cellPaddingY = 0;
            ...
            Log.d("Shortcut", "measureChild   cellPaddingX: " + cellPaddingX);
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != GONE) {
                ...
                int childLeft = lp.x;
                int childTop = lp.y;
                child.layout(childLeft, childTop, childLeft + lp.width, childTop + lp.height);//原配置长按图标不跳动
                //child.layout(childLeft, childTop + width / 2, childLeft + lp.width, childTop + lp.height + width / 2);//长按会跳动一下
                // Log.d("Shortcut", "onLayout: childIndex = " + i + " width:" + width + "  height:" + height);
                if (lp.dropped) {
                    lp.dropped = false;
