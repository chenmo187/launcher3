...
public class UiFactory {
    ...
    //清理所有应用的按钮传递点击事件
    public static void cleanAllTaskButton(Launcher launcher) {
        Log.d("Factory", "清理所有后台进程 ");
        RecentsView recentsView = launcher.getOverviewPanel();
        recentsView.dismissAllTasks(null);
    }

    public static boolean isLauncher3ctivityShow = false;

    //---------通知Launcher3  其他Launcher同步显示或隐藏清理所有图标---2021 07 02----------
    public static void setAllButtonVisibity(int currentTaskNum) {
        if (currentTaskNum == 0) {
            Log.d("Factory", "setAllButtonVisibity: 后台无进程");
            if (isLauncher3ctivityShow) {
                //原生Launcher
                if (mLauncher != null) {
                    mLauncher.mOverViewCleanButton.setVisibility(View.GONE);
                }
            } else {
                //其他Launcher
                RecentsActivity recentsActivity = RecentsActivityTracker.getCurrentActivity();
                if (recentsActivity != null) {
                    RecentsActivityTracker.getCurrentActivity().mOvCleanBtn.setVisibility(View.GONE);
                } else {
                    Log.d("Factory", "recentsActivity 不存在");
                }
            }
            return;
        }
        //其他第三方Launcher主界面显示清除按钮
        if (isLauncher3ctivityShow) {
            //原生Launcher3
            Log.d("Factory", "setAllButtonVisibity: 原生Launcher 后台管理");
            if (mLauncher != null) {
                mLauncher.mOverViewCleanButton.setVisibility(View.VISIBLE);
            }
        } else {
            Log.d("Factory", "setAllButtonVisibity: 第三方Launcher后台管理");
            RecentsActivity recentsActivity = RecentsActivityTracker.getCurrentActivity();
            if (recentsActivity != null) {
                recentsActivity.mOvCleanBtn.setVisibility(VISIBLE);
            } else {
                Log.d("Factory", "recentsActivity 不存在！！");
            }
        }
