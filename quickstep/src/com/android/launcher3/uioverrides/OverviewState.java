...
public class OverviewState extends LauncherState {
    ...
    public void onStateEnabled(Launcher launcher) {
        UiFactory.isLauncher3ctivityShow = true;
        Log.d("overviewState", "onStateEnabled ");
        RecentsView rv = launcher.getOverviewPanel();
        rv.setOverviewStateEnabled(true);
        //通知主界面
        launcher.recentViewEnable();//add by xiaoyu 2021 07 15

        AbstractFloatingView.closeAllOpenViews(launcher);
    }

    //原生Launcher3最近任务显示和消失的回调处,非原生Launcher3由RecentsActivity启动处理 2021 07 02 add by xiaoyu
    @Override
    public void onStateDisabled(Launcher launcher) {
        UiFactory.isLauncher3ctivityShow = false;
        Log.d("overviewState", "onStateDisabled");
        RecentsView rv = launcher.getOverviewPanel();
        rv.setOverviewStateEnabled(false);
        launcher.mOverViewCleanButton.setVisibility(View.GONE);//将原生Launcher3清除按钮隐藏
        launcher.recentViewDisable();//add by xiaoyu 2021 07 15
