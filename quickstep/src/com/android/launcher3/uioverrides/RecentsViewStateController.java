...
public class RecentsViewStateController implements StateHandler {
    private static String TAG = "RecentsViewStateController";
    ...
    public void setState(LauncherState state) {
        ...
        // mRecentsView.setTranslationYFactor(scaleTranslationYFactor[1]);//验证最近任务栏消失的动画
        if (state.overviewUi) {
    ...
    public void setStateWithAnimation(final LauncherState toState,
        ...
        if (!config.playAtomicComponent()) {
            ...
            Log.d(TAG, "setStateWithAnimation return");
            return;
        }
        ...
        if (mLauncher.getStateManager().getState() == OVERVIEW && toState == FAST_OVERVIEW) {
            ...
            Log.d(TAG, "setStateWithAnimation: 00000");
        }
        ...
        if (!toState.overviewUi) {
            // builder.addOnFinishRunnable(mRecentsView::resetTaskVisuals);//去掉这里后最近任务栏消失时候会从
            Log.d(TAG, "setStateWithAnimation 11111");//消失
        }

        if (toState.overviewUi) {
            Log.d(TAG, "setStateWithAnimation  222 ");//显示
            ...
            builder.play(updateAnim);//验证屏蔽退出任务栏的动画
            mRecentsView.updateEmptyMessage();
        }
    }
}
